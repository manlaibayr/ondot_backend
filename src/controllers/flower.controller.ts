...
import {FlowerHistoryRepository, MeetingProfileRepository, UsagePassRepository, UserRepository} from '../repositories';
...
export class FlowerController {
  ...
  async usagePassPurchase(
    ...
    const currentUser: UserCredentials = await this.getCurrentUser() as UserCredentials;
    if(currentUser.userFlower < passReqInfo.flower) throw new HttpErrors.BadRequest('플라워가 충분하지 않습니다.');
    // 이미 존재하는 이용권 확인
    ...
    await Promise.all([
      this.userRepository.updateById(currentUser.userId, {userFlower: currentUser.userFlower - passReqInfo.flower}),
      ...
      this.flowerHistoryRepository.create({flowerUserId: currentUser.userId, flowerContent: `${serviceType} ${passReqInfo.label} 이용권 구매`, flowerValue: -passReqInfo.flower}),
    ]);
    return {userFlower: currentUser.userFlower - passReqInfo.flower};
  }
}
