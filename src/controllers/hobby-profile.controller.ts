...
export class HobbyProfileController {
  ...
  async hobbyOtherProfileInfo(
    ...
    const hobbyChatList = await this.chatContactRepository.findOne(
      {where: {or: [{contactUserId: currentUser.userId, contactOtherUserId: userId}, {contactUserId: userId, contactOtherUserId: currentUser.userId}], contactServiceType: ServiceType.HOBBY}}
    )
  ...
  async hobbyRequestChat(
    ...
    return {contactId: chatContactInfo.id};
  }
}
